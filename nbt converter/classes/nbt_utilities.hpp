...
template <typename _Default, unsigned _Alt, class _Variant>
using variant_alternative_or_default_t =
	typename
		variant_alternative_or_default<_Default, _Alt, _Variant>
			::type;
	...
	using value_type =
		...
		,	std::conditional_t
			...
			,	compound_wrapper
				...
				,	variant_alternative_or_default_t
					<	std::string
					,	nbt::string, _Variant
					>
				>
...
# endif // NBT_UNIT_TEST_
	...
	public:
		...
		operator->(void) const
		{
			...
			if constexpr (_Alt == 8)
				return variant;
			else if constexpr (_Alt == 9)
				return std::unique_ptr<value_type>
				(	value_type
					(	std::get_if<_Alt>(variant)
					)
				);
			else if constexpr (variant_or_default::is_default)
				return variant;
			else
